use ash::vk::{CommandPool, DeviceQueueCreateInfo, CommandPoolCreateInfo, CommandBuffer, CommandBufferAllocateInfo, CommandBufferLevel};
...
impl LogicalDevice {
    ...
    pub(crate) fn allocate_command_buffer(&self,command_pool: CommandPool) -> Vec<CommandBuffer> {
        let create_info = CommandBufferAllocateInfo::builder().command_pool(command_pool).command_buffer_count(1).level(CommandBufferLevel::PRIMARY).build();
